<?php
    ...
    $sortColumn = isset($_POST['sort']) ? $_POST['sort'] : 'nama_menu'; // Default sorting by 'nama_menu'
    $sortOrder = isset($_POST['order']) ? $_POST['order'] : 'asc'; // Default sorting order is ascending

    // Construct the SQL query based on sorting preferences
    $query = "SELECT * FROM menu ORDER BY $sortColumn $sortOrder";
...
<html>
    ...
    <!-- Formulir pengurutan dengan gaya Bootstrap -->
    <form method="post" action="menu.php" class="form-inline mb-3">
        <label for="sort" class="mr-2">Sort by:</label>
        <select name="sort" id="sort" class="form-control mr-2" onchange="this.form.submit()">
            <option value="id_menu" <?php if ($sortColumn === 'id_menu') echo 'selected'; ?>>ID Menu</option>
            <option value="nama_menu" <?php if ($sortColumn === 'nama_menu') echo 'selected'; ?>>Nama Menu</option>
            <option value="harga" <?php if ($sortColumn === 'harga') echo 'selected'; ?>>Harga</option>
        </select>
        <select name="order" class="form-control mr-2" onchange="this.form.submit()">
                ...
                <tbody>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td><a href="edit_menu.php?id=<?php echo $row['id_menu']; ?>" class="btn btn-info">Edit</a></td>
                            <td><a href="delete_menu.php?id=<?php echo $row['id_menu']; ?>" class="btn btn-danger">Delete</a></td>
                        </tr>
                    <?php endwhile; ?>
                </tbody>
            </table>
