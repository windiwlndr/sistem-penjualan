...
$query = "SELECT * FROM order_table ORDER BY id_order DESC LIMIT 1"; // Mengurutkan berdasarkan ID order secara descending dan membatasi hasil ke 1
...
// Ambil ID order terakhir yang baru saja dibuat
$sql_last_order = "SELECT MAX(id_order) AS last_order_id FROM order_table";
$result_last_order = $conn->query($sql_last_order);
$row_last_order = $result_last_order->fetch_assoc();
$last_order_id = $row_last_order['last_order_id'];

// Ambil data order detail berdasarkan ID order
$sql_order_detail = "SELECT od.pk, m.nama_menu, od.jumlah, od.id_order, od.sub_total FROM order_detil od
                    JOIN menu m ON od.id_menu = m.id_menu
                    WHERE od.id_order = $last_order_id";
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    while ($row_order_detail = $result_order_detail->fetch_assoc()) {
                        echo "<tr>";
                        echo "<td>" . $no . ". </td>";
                        echo "<td>" . $row_order_detail['id_order'] . "</td>";
